		...
		<article>
			...
			<?php
				...
				if(isset($_GET['id']) && $_GET['id'] !== null){
					$complainid = $_GET['id'];
					//update database
					$query = mysqli_query(db(), "UPDATE complain SET addressed = 1 WHERE cmplainid = '$complainid'"); 
					if($query){
						echo "Complaint has been considered addressed!";
				...
				$query = mysqli_query(db(), "SELECT * FROM complain WHERE forwaded  =  1 ORDER BY cmplainid ASC"); 
								...
								<tr>
									...
									<td>
										<?php
											if($data['addressed'] == 0){
												?>
													<a href="view.php?id=<?= $data['cmplainid']; ?>"><button>Address</button></a>
												<?php
											}
											else{
												echo "<a href='#'>Addressed</a>";
											}
